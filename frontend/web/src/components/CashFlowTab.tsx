...
export default function CashFlowTab({ cashflow }: { cashflow: Row[] }) {
  ...
  return (
    <FinancialsCard title={t("cashFlowTab.titleBar")} latestLabel={t("cashFlowTab.latest")} rows={rows}>
      ...
      <ValueRow rows={rows} field="Free Cash Flow"      label={t("cashFlowTab.fcf")} highlight showGrowth signColor />
      ...
      <ValueRow rows={rows} field="Capital Expenditure"             label={t("cashFlowTab.capex")} indent />
      ...
      <ValueRow rows={rows} field="Repurchase Of Capital Stock" label={t("cashFlowTab.stockRepurchase")} indent zeroAsDash />
