...
function ValueRow({ rows, field, label, isTotal, isNeg, zeroAsDash, showGrowth, indent, highlight }: ValueRowProps) {
  ...
  return (
    <div className="flex items-stretch rounded-xl transition-colors hover:bg-white/[0.03]"
         style={{ background: highlight ? "rgba(0,168,94,0.06)" : undefined }}>
      <div className="shrink-0 flex items-center gap-2.5 px-6 py-3.5" style={{ width: 250, minWidth: 200 }}>
        ...
        <span className="text-[15px] leading-tight"
              style={{ fontWeight: highlight ? 800 : isTotal ? 800 : indent ? 500 : 600,
        ...
        return (
          <div key={i} className="flex-1 flex items-center justify-end gap-2.5 px-6 py-3.5">
            <span className="tabular-nums leading-none"
                  style={{ fontSize: highlight ? 19 : isTotal ? 18 : 15.5,
                           fontWeight: highlight ? 800 : isTotal ? 800 : isLast ? 700 : 500, color }}>
              {v != null ? fmtMoney(v) : "—"}
            </span>
            {growth != null && (
