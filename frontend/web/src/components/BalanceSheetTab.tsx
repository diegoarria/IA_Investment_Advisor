...
export default function BalanceSheetTab({ balance }: { balance: Row[] }) {
  ...
  const heroData = rows.map((r) => ({ label: fmtYear(String(r.period ?? "")), value: safeNum(r["Total Assets"]) }));

  return (
    <div className="px-1">
      <FinancialHeroChart title={t("balanceSheetTab.totalAssets")} data={heroData} color="#3b82f6" />
