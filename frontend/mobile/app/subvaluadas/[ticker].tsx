...
import {
  View, Text, TouchableOpacity, ScrollView, SafeAreaView, ActivityIndicator, Modal,
} from "react-native";
...
// Scoped dark navy/gold palette for this one screen — mirrors the web
// detail page's VI_THEME exactly (RN has no CSS custom properties, so this
// is passed explicitly as the `colors` prop to the same shared display
// components the rest of Subvaluadas uses, instead of useTheme()'s normal
// light/dark colors).
const viColors = {
...
export default function ValorIntrinsecoDetailScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: viColors.bg }}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 16, paddingVertical: 12 }}>
        <TouchableOpacity onPress={() => router.back()}><Ionicons name="chevron-back" size={22} color={viColors.text} /></TouchableOpacity>
        <Text style={{ fontSize: 12.5, color: viColors.textMuted, flexShrink: 1 }} numberOfLines={1}>
          {t("subvaluadas.title")} / {data ? `${name} (${ticker})` : ticker} / {t("subvaluadas.detail.breadcrumb")}
        </Text>
      </View>
