...
function tabLabel(tab: TabId, t: (k: string) => string): string {
  if (tab === "Portafolio") return t("patrimonio.tabs.portfolio");
  if (tab === "Watchlist") return t("patrimonio.tabs.watchlist");
  return t("patrimonio.tabs.simulator");
}
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16, gap: 12 }}>
      ...
      <View style={{ flexDirection: "row", gap: 8 }}>
        ...
        <View style={[ss.statCard, { flex: 1, backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          >
            {mask(fmtMoney(dayGain * fxRate, portfolioCurrency))}
          </Text>
      ...
      <View style={[ss.statCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <Text
          ...
          minimumFontScale={0.5}
        >
          {mask(fmtMoney(totalGain * fxRate, portfolioCurrency))}{" "}
      ...
      <View style={[ss.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={[ss.cardHeader, { borderBottomColor: colors.border }]}>
          <Text style={[ss.cardTitle, { color: colors.text }]}>{t("patrimonio.portfolioTab.positions", { count: positions.length })}</Text>
        ...
        {positions.length === 0 ? (
          <View style={ss.emptyState}>
            ...
            <Text style={[ss.emptyText, { color: colors.textMuted }]}>{t("patrimonio.portfolioTab.empty")}</Text>
          </View>
        ) : (
          positions.map((pos, i) => {
            const pr = prices[pos.ticker];
            ...
            return (
              ...
              >
                ...
                <View style={ss.rowRight}>
                  <Text style={[ss.rowValue, { color: colors.text }]}>{mask(fmtMoney(currentValue * fxRate, portfolioCurrency))}</Text>
...
function WatchlistTab({ prices, loading, colors }: { prices: PriceMap; loading: boolean; colors: any }) {
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16, gap: 12 }}>
      <View style={[ss.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={[ss.cardHeader, { borderBottomColor: colors.border }]}>
          <Text style={[ss.cardTitle, { color: colors.text }]}>{t("patrimonio.watchlistTab.title", { count: items.length })}</Text>
          {loading && <ActivityIndicator size="small" color={colors.accentLight} />}
        </View>
        ...
        {items.length === 0 ? (
          <View style={ss.emptyState}>
            ...
            <Text style={[ss.emptyText, { color: colors.textMuted }]}>{t("patrimonio.watchlistTab.empty")}</Text>
          </View>
        ) : (
          items.map((item, i) => {
