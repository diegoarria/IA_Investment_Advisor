        ...
        <View style={s.overlay}>
          <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <Text style={[s.body, { color: colors.textMuted }]}>
              Tuviste 30 días de acceso completo. Activa Premium para seguir usando el Arthur ilimitado, análisis avanzado de portafolio, Stress Test completo y más.
            </Text>
            <TouchableOpacity
...
function AppStack() {
  ...
  useEffect(() => {
    const sub = Notifications.addNotificationResponseReceivedListener((response) => {
      ...
      } else if (data.screen === "products_session") {
        // Friday 1:1-call upsell: Products screen with the session sheet open.
        router.navigate({ pathname: "/(tabs)/products", params: { open: "session" } } as any);
