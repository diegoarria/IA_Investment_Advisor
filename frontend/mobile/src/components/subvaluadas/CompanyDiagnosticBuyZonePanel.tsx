    ...
    <View style={{ marginTop: 14, borderRadius: 16, padding: 15, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border }}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10, paddingBottom: 13, marginBottom: 13, borderBottomWidth: 1, borderBottomColor: colors.border }}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          <Text style={{ fontSize: 15, fontWeight: "800", color: colors.text }} numberOfLines={1}>{t("subvaluadas.followAlert.title", { ticker: ticker.toUpperCase() })}</Text>
          <Text style={{ fontSize: 12, color: colors.textMuted }} numberOfLines={1}>{t("subvaluadas.followAlert.subtitle")}</Text>
        </View>
      </View>
      ...
      <View style={{ borderRadius: 14, padding: 13, backgroundColor: colors.bgRaised, borderWidth: 1, borderColor: colors.border }}>
        ...
        <Text style={{ fontSize: 18, fontWeight: "900", lineHeight: 23, color: colors.text }}>
          {t("subvaluadas.followAlert.belowLabel", { pct: selectedPct })}
        </Text>
        {targetPrice != null && (
          <Text style={{ fontSize: 13, marginTop: 3, color: colors.textDim }}>· {fmtPrice(targetPrice, currency)}</Text>
        )}
        <Text style={{ fontSize: 13, marginTop: 8, lineHeight: 18, color: colors.textMuted }}>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
          <View style={{ flex: 1, minWidth: "45%", borderRadius: 10, padding: 10, backgroundColor: colors.card }}>
            <Text style={{ fontSize: 10.5, fontWeight: "800", textTransform: "uppercase", color: colors.textMuted }} numberOfLines={1}>{t("subvaluadas.followAlert.currentPrice")}</Text>
            <Text style={{ fontSize: 16, fontWeight: "900", color: colors.text, marginTop: 2 }} numberOfLines={1} adjustsFontSizeToFit>{fmtPrice(price, currency)}</Text>
          </View>
          <View style={{ flex: 1, minWidth: "45%", borderRadius: 10, padding: 10, backgroundColor: colors.card }}>
            <Text style={{ fontSize: 10.5, fontWeight: "800", textTransform: "uppercase", color: colors.textMuted }} numberOfLines={1}>{t("subvaluadas.followAlert.intrinsicValue")}</Text>
            <Text style={{ fontSize: 16, fontWeight: "900", color: colors.text, marginTop: 2 }} numberOfLines={1} adjustsFontSizeToFit>{fmtPrice(intrinsicValue, currency)}</Text>
          </View>
          <View style={{ width: "100%", borderRadius: 10, padding: 10, backgroundColor: `${ACCENT_GREEN}14`, borderWidth: 1, borderColor: ACCENT_GREEN }}>
            <Text style={{ fontSize: 10.5, fontWeight: "800", textTransform: "uppercase", color: ACCENT_GREEN }} numberOfLines={1}>{t("subvaluadas.followAlert.marginOfSafety", { pct: selectedPct })}</Text>
            <Text style={{ fontSize: 17, fontWeight: "900", color: ACCENT_GREEN, marginTop: 2 }} numberOfLines={1} adjustsFontSizeToFit>{targetPrice != null ? fmtPrice(targetPrice, currency) : "—"}</Text>
          </View>
        </View>

        {error && <Text style={{ fontSize: 13, marginTop: 10, color: "#ef4444" }}>{error}</Text>}
        ...
        <View style={{ flexDirection: "row", gap: 8, marginTop: 12 }}>
          ...
          >
            ...
            <Text style={{ fontSize: 12.5, fontWeight: "800", color: "#fff", flexShrink: 1 }} numberOfLines={1} adjustsFontSizeToFit>
              {saving
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 7 }}>
          {otherPresets.map((pct) => (
            ...
            >
              ...
              {intrinsicValue != null && (
                <Text style={{ fontSize: 10.5, fontWeight: "700", color: colors.textMuted, marginTop: 3 }} numberOfLines={1} adjustsFontSizeToFit>
          ...
          {!customOpen ? (
            ...
            <View style={{ width: "100%", borderRadius: 12, padding: 10, flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: colors.bgRaised, borderWidth: 1, borderColor: colors.border }}>
              <TextInput
                ...
                style={{ flex: 1, fontSize: 13.5, color: colors.text, paddingVertical: 4 }}
              />
              <TouchableOpacity onPress={handleCustomSubmit}>
                <Text style={{ fontSize: 13, fontWeight: "800", color: ACCENT_GREEN }}>{t("subvaluadas.followAlert.customApply")}</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
