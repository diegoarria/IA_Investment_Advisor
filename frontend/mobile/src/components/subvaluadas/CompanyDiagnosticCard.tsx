  ...
  return (
    <View style={{ flexDirection: "row", alignItems: "flex-end", justifyContent: "space-between", gap: 10, marginBottom: 8 }}>
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text style={{ fontSize: 14, fontWeight: "800", color: colors.text }} numberOfLines={1}>{title}</Text>
        {subtitle && <Text style={{ fontSize: 11, color: colors.textMuted, marginTop: 2 }}>{subtitle}</Text>}
      </View>
      {icon}
    </View>
  );
    ...
    <View>
      ...
      <View style={{ borderRadius: 18, padding: 16, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border }}>
        <View style={{ flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 12 }}>
          <View style={{ flex: 1, minWidth: 0 }}>
            <View style={{ flexDirection: "row", alignItems: "baseline", flexWrap: "wrap", gap: 6 }}>
              <Text style={{ fontSize: 22, fontWeight: "900", color: colors.text }}>{data.ticker}</Text>
              <Text style={{ fontSize: 13.5, fontWeight: "600", color: colors.textSub }} numberOfLines={1}>{data.companyName}</Text>
            </View>
            <Text style={{ fontSize: 12, marginTop: 3, color: colors.textMuted }}>{data.sector} · {data.exchange}</Text>
          </View>
          <View style={{ alignItems: "flex-end" }}>
            ...
            <Text style={{ fontSize: 11, fontWeight: "800", textTransform: "uppercase", color: scoreColor(data.score), marginTop: 4, textAlign: "right" }}>
              {data.scoreLabel}
            </Text>
          </View>
        </View>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 7, marginBottom: 14 }}>
          {data.badges.map((b) => (
            <View key={b} style={{ paddingHorizontal: 11, paddingVertical: 7, borderRadius: 12, backgroundColor: "#6366F11f", borderWidth: 1, borderColor: "#6366F1" }}>
              <Text style={{ fontSize: 11.5, fontWeight: "800", color: "#6366F1" }}>{b}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={{ flexDirection: "row", gap: 8, marginBottom: 14 }}>
          <View style={{ flex: 1, borderRadius: 12, padding: 11, backgroundColor: colors.bgRaised }}>
            <Text style={{ fontSize: 10, fontWeight: "800", textTransform: "uppercase", color: colors.textMuted }}>{t("companyDiagnostic.kpi.currentPrice")}</Text>
            <Text style={{ fontSize: 17, fontWeight: "900", color: colors.text, marginTop: 2 }}>{fmtPrice(data.valuation.currentPrice)}</Text>
          </View>
          <View style={{ flex: 1, borderRadius: 12, padding: 11, backgroundColor: colors.bgRaised }}>
            <Text style={{ fontSize: 10, fontWeight: "800", textTransform: "uppercase", color: colors.textMuted }}>{t("companyDiagnostic.kpi.fairValue")}</Text>
            <Text style={{ fontSize: 17, fontWeight: "900", color: "#4FA695", marginTop: 2 }}>{fmtPrice(data.valuation.baseFairValue)}</Text>
          </View>
        </View>

        <View style={{ borderRadius: 12, padding: 11, marginBottom: 14, backgroundColor: colors.bgRaised, borderLeftWidth: 3, borderLeftColor: colors.accent }}>
          <Text style={{ fontSize: 13, lineHeight: 19, color: colors.text, fontStyle: "italic" }}>&ldquo;{data.oneLinerPitch}&rdquo;</Text>
        </View>

        {verdictStatus && (
          <View style={{ alignItems: "center", justifyContent: "center", paddingVertical: 10, marginBottom: 12 }}>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
              <Text style={{ fontSize: 20, fontWeight: "900", color: VERDICT_COLOR[verdictStatus.verdict] }}>
        ...
        {(data.valuation.fcfAssumptions || data.valuation.waccDetails) && (
          <View style={{ marginTop: 8 }}>
            <TouchableOpacity onPress={() => setAssumptionsOpen((o) => !o)}>
              <Text style={{ fontSize: 10.5, color: colors.textMuted, textDecorationLine: "underline" }}>
            ...
            {assumptionsOpen && (
              <View style={{ marginTop: 8, gap: 6 }}>
                {data.valuation.fcfAssumptions && (
                  <>
                    <Text style={{ fontSize: 10.5, color: colors.textSub }}>
                ...
                {data.valuation.fcfAssumptions?.methodology_note && (
                  <Text style={{ fontSize: 10.5, color: colors.textDim }}>{data.valuation.fcfAssumptions.methodology_note}</Text>
                )}
              </View>
            )}
          </View>
      ...
      {data.investmentThesis && (
        <View style={{ marginTop: 14, borderRadius: 18, padding: 15, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.accent }}>
          <DiagSectionHeader
            ...
            colors={colors}
          />
          <Text style={{ fontSize: 13, lineHeight: 19, color: colors.textSub, marginTop: 6 }}>
      ...
      <View style={{ marginTop: 14, borderRadius: 18, padding: 15, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border }}>
        ...
        <View style={{ marginTop: 6, gap: 8 }}>
          {methodologyParagraphs.map((p, i) => (
            <Text key={i} style={{ fontSize: 12.5, lineHeight: 18, color: colors.textSub }}>{p}</Text>
          ))}
        </View>
      </View>
