...
// Nuvos AI Fair Value Engine redesign, Incremento 16 — no longer compares
// two independent methods (intrinsic value vs. fair_value_engine's
// justified-multiple estimate): fair_value_engine is retired as a
// standalone valuation (screener.py stops computing/emitting it; its
// adjustment functions live on as a direct dependency of
// exit_multiple_engine.py since Incremento 1). One real number, not a
// "do 2 methods roughly agree" framing.
export function FinalResultPanel({
  intrinsicValue, price, confidence, colors,
}: {
  ...
  return (
    <View style={{ borderWidth: 1, borderRadius: 16, padding: 14, borderColor: colors.border, backgroundColor: colors.card }}>
      ...
      {price !== null && (
        <Text style={{ fontSize: 11, color: colors.textSub, marginBottom: 8 }}>{t("subvaluadas.finalResult.currentPrice")}: <Text style={{ fontWeight: "800", color: colors.text }}>${price.toFixed(2)}</Text></Text>
      )}
