 ...
 *
 * This is an intentional duplicate of frontend/web/src/lib/dcfCalculator.ts
 * — this repo has no shared package between web and mobile, so the same
 * pure formula is verified once (Vitest, on the web side) and kept
 * identical here by inspection rather than a second test runner.
 *
...
/**
 * Fase 1.5, Incremento 17 (mobile parity with web's Incremento 15) —
 * year-by-year projection with FCF growth fading linearly from `g` to `gt`
 * across the explicit window, replacing the previous CONSTANT-growth
 * closed-form annuity (flat `g` for all `n` years, then a discrete jump to
 * `gt` for the terminal value). Same mathematical behavior
 * `project_driver_based_dcf` (the backend engine this app's real valuation
 * now uses) applies to revenue growth — kept byte-for-byte identical to the
 * web copy, see this file's module docstring.
 *
 ...
 * 1.5, Incremento 14 dedup: matches the web copy and the single backend
 * convention (numeric_helpers.py::calc_margin_of_safety). */
