...
# v8 — bumped for methodology audit round 5 — mandatory per-share Fair
# Value Engine (see /Users/diegoarria/.claude/plans/cosmic-munching-
# crown.md): GQV's growth-evidence hierarchy now has a new top-priority
# per-share-compounded tier (real revenue CAGR × real buyback yield),
# replacing the old EPS-CAGR-based buyback discount for companies with
# real share-count history — changes `fair_pe`/Bear-Base-Bull scenarios
# for real tickers, especially buyback-heavy ones (AAPL, META, ACN). Also
# the legacy DCF scenarios and Reverse DCF now use a real per-year
# shrinking share count instead of one averaged scalar. A v7 entry
# predates all of this.
# v7 — bumped for methodology audit round 3 (see /Users/diegoarria/.claude/
# plans/cosmic-munching-crown.md): ROIC now falls back to operating
# invested capital when buyback-compressed equity would otherwise blow it
# up unbounded (real case: 1500%+), changing `avg_roic_pct` for affected
# tickers. A v6 entry predates this guard.
# v6 — bumped for methodology audit round 2 (see /Users/diegoarria/.claude/
# plans/cosmic-munching-crown.md): net cash now includes Long Term
# Investments (was understating real liquidity for companies like Apple
# that hold long-duration marketable securities), affecting net-cash-
# dependent multiples (EV/EBITDA, EV/FCF). Also adds real forward P/E and
# P/E-on-normalized-earnings fields. A v5 entry predates all of this.
# v5 — bumped for the FCF maintenance/growth-CapEx normalization fix (see
# /Users/diegoarria/.claude/plans/cosmic-munching-crown.md, methodology
# audit of the external feedback email): `_fcf_margin_adjustment`'s input
# now uses maintenance-capex-only FCF instead of OCF-minus-total-CapEx,
# which changes the Fair P/E — and therefore the Fair Value scenarios —
# for any capex-heavy candidate (e.g. AI-infra/data-center names). A v4
# entry was built with the old, undifferentiated FCF margin.
