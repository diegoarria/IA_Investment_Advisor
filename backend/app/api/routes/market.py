...
def _fetch_stock_detail(symbol: str) -> dict:
    ...
    # Delegates to financial_data_service's provider chain (Fiscal.ai → FMP →
    # yfinance, tried per-call with automatic fallback) instead of a bespoke
    # fetch here that leaned on yfinance for quarterly data unconditionally
    # and had no fallback beyond it — yfinance rate-limits constantly, and
    # this is the same resilient pipeline that already powers the Financieros
    # tab via /financials, so both surfaces now share one source of truth.
    from app.services.financial_data_service import get_financials as _get_rich_financials
    _fin = _get_rich_financials(symbol, limit=5)
    income_annual  = _fin["incomeStatement"]["annual"]
    balance_annual = _fin["balanceSheet"]["annual"]
    financials = {
        "income":   _fin["incomeStatement"],
        "balance":  _fin["balanceSheet"],
        "cashflow": _fin["cashFlow"],
        "source":   _fin["provider"],
    }
