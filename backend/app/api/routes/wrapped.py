...
async def compute_wrapped(user_id: str, now: datetime, year: int) -> dict:
    """The full annual Wrapped computation — extracted out of the
    /api/wrapped/annual route so job_wrapped_email (worker.py) can build the
    exact same report a user sees when opening Wrapped in-app, instead of a
    separately-written approximation that could drift (same discipline as
    build_weekly_email_for_user re-using the real weekly-email code path)."""
    db      = get_supabase()
...
async def get_wrapped(
    ...
    try:
        return await compute_wrapped(user["id"], now, year)
    except Exception:
        if _test_bypass:
